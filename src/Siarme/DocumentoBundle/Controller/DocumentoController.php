...
/**
    ...
    public function newAction(Request $request, $tramite_id, $slug)
    {
     ...
     /*** $estado = (($expediente->getDatoAt()->getTurno() == true) or ($expediente->getDatoAt()->getEstado() == "Con Turno") );
        ...
        if ($documento->getSlug() == "planificacion"){
            if (count($documento->getTramite()->getPlanificacion()) > 0) {
                $msj = "Existen RUBROS planificados, si carga nuevamente la planilla, se perderán los PEDIDOS relacionados a los rubros existentes";
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($documento->getTipoDocumento()->getEsArchivo() == true && !empty($file) && $file != null) {
                ...
                #---Agrego los Items de la nota al PEDIDO
                if ($documento->getSlug() == "nota-pedidopdf"){
                   // Llamo a la funcion "agregarItem" que devuelve el texto del documento o false en caso de error.
                    //$msj = $this->itemNP($fileName, $tramite);
                    $ext = pathinfo($fileName, PATHINFO_EXTENSION);
                   
                    if ( in_array($ext, ["pdf"])) {
                         $msj = $this->itemNpdf($fileName, $tramite);
                         $this->get('session')->getFlashBag()->add('mensaje-info', $msj);
                    } else {
                        $msj = "Debes seleccionar una Archivo PDF, la extencion $ext no corresponde";
                        $this->get('session')->getFlashBag()->add('mensaje-warning', $msj);
                        $msj = false;
                    }
                ...
                #---Agrego los RUBROS PLANIFICADOS DE DPPR
                if ($documento->getSlug() == "planificacion"){
                   // Llamo a la funcion "agregarItem" que devuelve el texto del documento o false en caso de error.
                    ...
                    if ( in_array($ext, ["xlsx", "xls"])) {
                       $msj = $this->planificacionTrimestral($fileName, $tramite);
                    } else {
                        $msj = "Debes seleccionar una Archivo Excel, la extencion $ext no corresponde";
                        $this->get('session')->getFlashBag()->add('mensaje-warning', $msj);
                        $msj = false;
   ...
   /**
     ...
     *
     */
     public function itemNpdf($fileName, $tramite)           
    {
            $pdfFile = $this->getParameter('archivos').'/'.$fileName;

            if (!is_readable($pdfFile)) {
                throw new \Exception("Error: el archivo no existe o no es legible: $pdfFile");
            }

            $c = curl_init();

            $cfile = curl_file_create($pdfFile, 'application/pdf');
            $apikey = 'd0rihwa77x4v'; // tu API key
            ...
            if (curl_errno($c) > 0) {
                throw new \Exception('Error llamando a PDFTables: '.curl_error($c));
            }

            curl_close($c);

            // Nombre final del archivo Excel
            $newFileName = '1_Nuevo.xlsx';
            $xlsxPath = $this->getParameter('archivos').'/'.$newFileName;

            // Guardar el archivo Excel
            if (file_put_contents($xlsxPath, $result) === false) {
                throw new \Exception("Error al guardar el archivo Excel en: $xlsxPath");
            }

            $msj = "Convertido a Excel correctamente: $newFileName";
            $this->get('session')->getFlashBag()->add('mensaje-info', $msj);
     ...
     public function itemNPT($fileName, $tramite)           
    {
       $documentoXls = IOFactory::load($this->getParameter('archivos').'/'.$fileName);
        ...
        if (!$escolETenica){
            $msj = "No se encontró la columna de ESPECIFICACION TÉCNICA";
            $this->get('session')->getFlashBag()->add('mensaje-warning', $msj);
        } 
                ...
                if ($esItem == true ) {
                        ...
                        foreach ($fila->getCellIterator() as $celda) {
                            ...
                            try {
                                ...
                                // En caso de error al calcular la fórmula, obtenemos el valor previamente calculado
                                $valorRaw = $celda->getOldCalculatedValue();
                            }
                              switch ($c) {
                                case $colId:  
                                     $itemNumero = $valorRaw;                                           
     ...
     *
     */
     public function planificacionTrimestral($fileName, $tramite)           
    {
       $documentoXls = IOFactory::load($this->getParameter('archivos').'/'.$fileName);
        ...
        $esHoja = false;
        for ($i=0; $i < 10; $i++) { 
            $columnaValorA= trim($hojaActual->getCell("A".$i)->getCalculatedValue());
            $columnaValorB= trim($hojaActual->getCell("B".$i)->getCalculatedValue());
            if ((strpos($columnaValorA, "RUBRO") !== false) or (strpos($columnaValorB, "RUBRO") !== false)) {
                    $escolRubro = false;
                    $escolDescripcion = false;
                    $escolImporte = false;
                    $escolConcentra = false;
                    $colId = 1;
                    $esHoja = true;
                    for ($col=1; $col < 10; $col++) {
                        ...
                        if (strpos($columnaValor, "IMPORTE") !== false) {
                             $escolImporte = true;
                             $colImporte = $col;
                        }

                
                $npt= ($escolRubro and  $escolDescripcion and $escolImporte );
                $filaItem= $i;
             }
        }
     }
        ...
        if (!$esHoja){
            $msj = "Se encontraron  $numeroHojas hojas , Verifique que sea la 1° hoja la Nota TRIMESTRAL (no se encontró la columna RUBRO COMPRAR)";
            $this->get('session')->getFlashBag()->add('mensaje-warning', $msj);
            return false;
        } 
        ...
        $f=0;
        $totalItems=0;
        $planificaciones = $tramite->getPlanificacion();
        foreach ( $planificaciones as $rubro) {
            $em->remove($rubro);
             $em->flush();
        }
       foreach ($hojaActual->getRowIterator() as $fila) {
        $f = $f + 1;
            ...
            // if verifica que sea item por ID
            if ($f > $filaItem) {
                $columnaValorA= trim($hojaActual->getCell("A".$f)->getCalculatedValue());
                $columnaValorB= trim($hojaActual->getCell("B".$f)->getCalculatedValue());
                //if verifica que sea una fila de item 
                $esItem = false;
                if ( (!(empty($columnaValorA))) and (!(empty($columnaValorB))) ) {
                        

                           $item = new Planificacion();
                           $item->setTramite($tramite);
                           $item->setTrimestre($tramite->getTrimestre());
                             ...
                             $valorRaw = trim($celda->getCalculatedValue());
                              ...
                              switch ($c) {
                                case $colRubro:
                                 ...
                                 case $colDescripcion:
                                      $escolDescripcion = (empty($valorRaw)) ? false : true;
                                      $item->setDescripcion($valorRaw);
                                      // $item->setItem(Util::getSlug($valorRaw));
                                      //$item->setCodigo($item->getNumero()." - n/d ");
                                     break;
                                 case $colImporte:
                                      $escolImporte = (empty($valorRaw)) ? false : true;
                                      $item->setImporte($valorRaw);
                                     break;

                                } //switch
                        } // foreach columna

                        $esItem= ($escolRubro and  $escolDescripcion and $escolImporte);
                        ...
                        if ($esItem == true ) {
                            ...
                            if (!$escolRubro) {
                                $msj = $msj . "- RUBRO COMPRAR ";
                            }
                            if (!$escolDescripcion) {
                                $msj = $msj . "- DESCRIPCION ";
                            }
                            if (!$escolImporte) {
                                $msj = $msj . "- IMPORTE ESTIMADO ";
                            }
