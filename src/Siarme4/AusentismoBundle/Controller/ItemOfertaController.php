...
class ItemOfertaController extends Controller
{
     ...
     *
     * @Route("/item-cantidad/{id}", name="item_adjudicado_cantidad_ajax")
     * @Method("POST")
     */
    public function itemAdjudicadoCantidadAction(Request $request, $id)
    {

        // Verificar si la petición es AJAX (opcional, pero buena práctica)
        if (!$request->isXmlHttpRequest()) {
            return new JsonResponse(['success' => false, 'message' => 'Solo se permiten peticiones AJAX.'], JsonResponse::HTTP_BAD_REQUEST);
        }

        // Obtener los datos del cuerpo de la petición JSON
        $data = json_decode($request->getContent(), true);

        // Validar si la 'cantidad' está presente y es un entero positivo
        if (!isset($data['cantidad']) || !is_int($data['cantidad']) || $data['cantidad'] < 0) {
            return new JsonResponse(false); // O podrías enviar un JSON con más detalles: ['success' => false, 'message' => 'Cantidad inválida']
        }
        $cantidad = $data['cantidad'];

        // Aquí iría tu lógica de negocio para guardar la cantidad
        // Por ejemplo, podrías buscar la entidad por $tipoid y $tipo,
        // actualizar la cantidad y persistir en la base de datos.
         
         $em = $this->getDoctrine()->getManager();
         $item = $em->getRepository('AusentismoBundle:ItemProcesoAdjudicado')->find($id);
         $item->setCantidad($cantidad);

        try {
            $em->flush();
        } catch (\Throwable $e) {
            //return new Response('Error al guardar: ' . $e->getMessage(), 500);
            return new JsonResponse(false);
        }
             //return new Response('Guardado exitosamente', 200);
    ...
    public function adjudicarAction(Request $request, ItemOferta $item)
    {
            ...
            $em = $this->getDoctrine()->getManager();
                ...
                $itemsAd = $item->getItemProceso()->getItemProcesoAdjudicado();
