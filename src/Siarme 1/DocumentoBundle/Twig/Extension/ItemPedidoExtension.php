...
class ItemPedidoExtension extends \Twig_Extension
{
    ...
    public function itemDuplicadoFilter($item)
    {
       ...
       $repository = $this->em->getRepository('AusentismoBundle:ItemPedido');
        ...
        $fecha1 = $item->getTramite()->getFechaDestino();
         $fecha1->modify('30 day');
        $query = $repository->createQueryBuilder('i'); 
               $query->innerJoin('i.tramite' , 't')
                    ...
                    ->andWhere('t.fechaDestino <= :fecha1')
  ...
  if ($tipo == "ipp" and $adjudicado) {
            ...
            $dql = 'SELECT i AS itemP, i.ipp AS ipp, i.rubro AS rubro, COUNT(i.id) AS cantidad, SUM(i.cantidad * i.precio) AS presupuesto, SUM(io.cantidadAdjudicada * io.precio) AS adjudicado
                    FROM AusentismoBundle:ItemPedido i 
            ...
            $dql = 'SELECT i AS itemP, i.ipp AS ipp, i.rubro AS rubro, COUNT(i.id) AS cantidad, SUM(i.cantidad * i.precio) AS presupuesto
                    FROM AusentismoBundle:ItemPedido i 
                    WHERE i.tramite = :pedido 
                    GROUP BY i.ipp
                    ORDER BY i.ipp ASC';
