...
/**
        ...
        while ($esOferta){
            ...
            foreach ($filas as $fila) {
                ...
                if ($f == 8) {
                    ...
                    $oferente = strtoupper($hojaActual->getCellByColumnAndRow($colOferta, 8)->getValue());
 ...
 /**       foreach ($proceso->getItemProceso() as $itemProceso ){
                        $items = $this->obtenerItemPedido($itemProceso);
                        if (count($items)== 1){
                            foreach ($items as $itemPedido ){
                            ...
                            $cantidadP = 0;
                            foreach ($items as $itemPedido ){
                                $cantidadP = $cantidadP + $itemPedido->getCantidad();
                            }
                            if ($itemProceso->getCantidad()== $cantidadP) {
                                foreach ($items as $itemPedido ){
                                   //$itemProceso->addItemPedido($itemPedido);
                                   $itemPedido->setItemProceso($itemProceso);
                                }
...
*/
     $repository = $em->getRepository('AusentismoBundle:ItemPedido');
        ...
        if (!empty($msjExiste)) {
            $msjExiste = "<p  style='color:red;'>Se actualizaron item existentes: ".$msjExiste."</p>";        
     ...
     * @return array
     */
    public function obtenerItemPedido($item)
    {
        $em = $this->getDoctrine()->getManager();
       ...
       $repository = $em->getRepository('AusentismoBundle:ItemProceso');
        ...
        if (count($items) > 1) {
               ...
               $query->Where('i.codigo = :codigo')
                    ...
                    ->andWhere('i.itemPedido IS EMPTY')
                    ->andWhere('i.proceso = :proceso')
                    ->setParameter('codigo',  $codigo)
                    ->setParameter('cant',  $cantidad)
