...
use Siarme\AusentismoBundle\Entity\ItemProcesoAdjudicado;
...
class TramiteController extends Controller
{
    ...
    public function indexAction($anio = null)
    {
       ...
       if ($usuario->getDepartamentoRm()->getSlug() == "sca") {
        ...
        $expedientesPendientes = array();
        foreach ($expedientes as $expediente) {
            $tramites1=$expediente->getTramite();
            $estado= false;
            foreach ($tramites1 as $tramite1) {
                    if ( $tramite1->getDepartamentoRm() == $usuario->getDepartamentoRm()) {
                          $estado= true;
                    }
            }
...
/**-------------------------------</ ACCIONES PARA EXPEDIENTES------------------------------------------------*/
    ...
    public function newAction(Request $request, $tipo_tramite_id = null)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($tramite->getTipoTramite()->getSlug() == "tramite_planifica" )  {
                switch ($tramite->getTrimestre()) {
                    case 1 :
                        $estado= $em->getRepository('ExpedienteBundle:EstadoTramite')->find(48);
                        $tramite->setEstadoTramite($estado);
                        break;
                    case 2 :
                        $estado= $em->getRepository('ExpedienteBundle:EstadoTramite')->find(49);
                        $tramite->setEstadoTramite($estado);
                        break;
                    case 3 :
                        $estado= $em->getRepository('ExpedienteBundle:EstadoTramite')->find(50);
                        $tramite->setEstadoTramite($estado);
                        break;
                    case 4 :
                        $estado= $em->getRepository('ExpedienteBundle:EstadoTramite')->find(51);
                        $tramite->setEstadoTramite($estado);
                        break;
                }
            }
  ...
  /**
     ...
     * @Method({"GET", "POST"})
     */
    public function controlNewAction(Request $request, Tramite $pedido)
    {
        $em = $this->getDoctrine()->getManager();
        $tramite = new Tramite();
        ...
        $tramite->setDepartamentoRm( $this->getUser()->getDepartamentoRm());
        //$tipoTramite = $em->getRepository('ExpedienteBundle:TipoTramite')->find(1);
        $tipoTramite = $em->getRepository('ExpedienteBundle:TipoTramite')->findOneBySlug("tramite_control");
        $tramite->setTipoTramite($tipoTramite);
    ...
    public function showAction(Tramite $tramite)
    {
        ...
        //VERIFICO SI HAY UN EXPEDIENTE O SOLICITUD INICIADA
        $movimientoPendiente = $em->getRepository('ExpedienteBundle:Movimiento')->findOneBy(array('departamentoRm'=>null,'usuario' => $usuario));
       // $tramite = $em->getRepository('ExpedienteBundle:Tramite')->findTramite($tramite1->getId());
        $items = "";
  ...
  /**
   ...
   /**
    ...
    public function adjudicarAction(Request $request, Tramite $proceso)
    {
        ...
        // Elimino los items adjudicados
        $itemsAd = $proceso->getItemProcesoAdjudicado();
        foreach ($itemsAd as $item ) {
             $em->remove($item);
             $em->flush();
        }
       ...
       //recupero las pagina anterior 
        ...
        /**
     * 
     * @param item $itemOferta The ItemOferta entity
     *
     */
    private function crearItemAdjudicado($itemOferta)
    {       
            $msj = false;
            $em = $this->getDoctrine()->getManager();
            $cantidadPedida = $itemOferta->getItemProceso()->getCantidadPedida();
            $cantidadAdjudicada = $itemOferta->getCantidadAdjudicada();
            $itemProceso = $itemOferta->getItemProceso();
            $itemPedidos = $itemOferta->getItemProceso()->getItemPedido();
            $cantidadTotal = 0;

          //creo los items adjudicados
          if ($itemOferta->getAdjudicado()) {              
                $msj = true;
                foreach ($itemPedidos as $itemPedido ) {

                        $cantidad = ($itemPedido->getCantidad()/$cantidadPedida)*$cantidadAdjudicada;
                        $cantidad = round($cantidad);
                        $precio = $itemOferta->getPrecio();

                        $itemPA = new ItemProcesoAdjudicado();

                        $itemPA->setNumero($itemOferta->getNumero());
                        $itemPA->setCantidad($cantidad);
                        $itemPA->setPrecio($precio);
                        $itemPA->setProceso($itemOferta->getProceso());
                        $itemPA->setOferta($itemOferta->getOferta());
                        $itemPA->setItemOferta($itemOferta);
                        $itemPA->setItemProceso($itemProceso);
                        $itemPA->setItemPedido($itemPedido);
                         $em->persist($itemPA);
                         $em->flush();
    ...
    public function estadoCambiarAction(Request $request, Tramite $tramite, $estado_id)
    {
        ...
        if ($tramite->getTipoTramite()->getSlug() == "tramite_planifica" )  {
                switch ($estado->getId()) {
                    case 48 :
                        $tramite->setTrimestre(1);
                        break;
                    case 49 :
                        $tramite->setTrimestre(2);
                        break;
                    case 50 :
                        $tramite->setTrimestre(3);
                        break;
                    case 51 :
                        $tramite->setTrimestre(4);
                        break;
                }
        }
        $tramite->setEstadoTramite($estado);
        ...
        // Envio mensajes SMS y Emails a usuarios
        if ($tramite->getTipoTramite()->getSlug() == "tramite_despacho" and in_array($estado->getSlug(), ["firmada","me_adjudicar", "concluido"])) {
            
            $tipoResolucion = "Llamado";
            $mensaje = "Resolucion de ".$tipoResolucion." firmada para ".$tramite->getNumeroComprar()." en compras-sca.online";
            if ($estado->getSlug() == "me_adjudicar") {
                    $tipoResolucion = "Adjudicacion";
                    $mensaje = "Proyecto de Resolucion de ".$tipoResolucion." firmada SC para ".$tramite->getNumeroComprar()." en compras-sca.online";
            }
            if ($estado->getSlug() == "concluido") {
                    $tipoResolucion = "Adjudicacion";
                    $mensaje = "Resolucion de ".$tipoResolucion." firmada MHOP para ".$tramite->getNumeroComprar()." en compras-sca.online";
            }
            $i=1;
            foreach ($tramite->getTarea() as $tarea) {
                if ($i > 1) {
                        ...
                        $telefono = $tarea->getUsuario()->getAgente()->getTelefonoMovil();
                            if (Util::esTelefono($telefono)) {
                                $respuesta = $this->sendSms($telefono, $mensaje);
                                if ($respuesta) {
   ...
   /**
     * Elimina todos rubros planificados de una Planificacion.
     *
     * @Route("/{id}/delete-all", name="item_planificacion_delete_all")
     * @Method("GET")
     */
    public function deleteAllAction(Request $request, Tramite $tramite)
    {     
        $em = $this->getDoctrine()->getManager();
        foreach ($tramite->getPlanificacion() as $item) {
                $em->remove($item);
        }
        $em->flush();
        $msj= "Has eliminado los Rubros Planificados";
        $this->get('session')->getFlashBag()->add(
                    'mensaje-info',
                    $msj);
        $referer= $request->headers->get('referer');
 ...
 public function historial($entidad, $accion, $msj, $tipo)
     ...
     public function sendSms($telefono, $messageBody)
    {
        ...
        $twilio_phone_sms = $this->container->getParameter('twilio_phone_sms');
