...
//dump($comentarios); exit();
     ...
     *
     * @Route("ajax/{tipoid}/{tipo}/new", name="comentario_ajax_new")
     * @Method({"GET", "POST"})
     */
    public function newAjaxAction(Request $request, $tipoid = null, $tipo= null)
    {
        $comentarioTexto = $request->request->get('comentario-texto'); // Obtener el valor del textarea
        ...
        return new JsonResponse(['id' =>$comentario->getId(), 'texto' => $comentario->getTexto()]);
        // Ejemplo de un error
        // return new JsonResponse(false, 400); // Devuelve false y un código de error
    }
