...
class CuponExtension extends \Twig_Extension
{
    public function getFilters()
    {
        return array(
            ...
            new \Twig_SimpleFilter('mostrar_como_cuil', array($this, 'mostrarComoCuil'), array('is_safe' => array('html'))),
        );
    }
...
EOJ;
    ...
    /**
     * Muestra como una CUIL HTML el contenido de texto al que se
     * aplica el filtro. 
     *
     * @param string $value El texto que se transforma
     * @param string $tipo  
     ...
     * @return string
     */
    public function mostrarComoCuil($value)
    {
        $ultimo = substr($value, -1);
        $primero = substr($value, 0, 2);
        $medio = substr($value, 2, -1);
        $html = $primero."-".$medio."-".$ultimo;
        return $html;
    }
