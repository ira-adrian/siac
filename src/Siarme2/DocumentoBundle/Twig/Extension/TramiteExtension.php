...
class TramiteExtension  extends \Twig_Extension
{
    ...
    public function getFilters()
    {
        return [
            ...
            new Twig_SimpleFilter('planificacion', [$this, 'planificacion']),
            new Twig_SimpleFilter('rubro_planificado', [$this, 'rubroPlanificado']),
            new Twig_SimpleFilter('pedidos_planificados', [$this, 'pedidosPlanificados']),
        ];
    }
     ...
     * @return boolean
     */
    public function planificacion($tramite)
    {

        $anio = ($tramite->getFechaDestino())->format('Y');
        $fechaDesde = $anio."/01/01";
        $fechaDesde = new \DateTime($fechaDesde);
        $fechaDesde = $fechaDesde->format('Y/m/d');
        ...
        $fechaHasta = new \DateTime($fechaHasta);
        $fechaHasta = $fechaHasta->format('Y/m/d');
        $trimestre = $tramite->getTrimestre();
        $organismo = $tramite->getOrganismoOrigen();
        $slug = "tramite_planifica";
        $dql = 'SELECT t
                FROM ExpedienteBundle:Tramite t 
                JOIN t.tipoTramite tt
                WHERE t.organismoOrigen = :organismo 
                AND t.trimestre = :trimestre
                AND t.fechaDestino >= :fechaDesde 
                AND t.fechaDestino <= :fechaHasta 
                AND tt.slug = :slug';
        $query = $this->em->createQuery($dql);
     ...
     * @return boolean
     */
    public function rubroPlanificado($tramite)
    {
        $dql = 'SELECT p
                FROM ExpedienteBundle:Planificacion p 
                WHERE p.pedido = :t';
        $query = $this->em->createQuery($dql);
        $query->setParameter('t', $tramite->getId());
     ...
     * @return boolean
     */
    public function pedidosPlanificados($tramite)
    {
...
/**------------------------------TRAMITE DEL USUARIO---------------------------------*/
     ...
     * @return boolean
     */
    public function rubroPlanificadoTrimestre($anio, $trimestre = 1)
    {
        $fechaDesde = $anio."/01/01";
        $fechaDesde = new \DateTime($fechaDesde);
        $fechaDesde = $fechaDesde->format('Y/m/d');
        ...
        $slug = "tramite_planifica";
        $dql = 'SELECT i 
                    FROM ExpedienteBundle:Planificacion i 
                    ...
                    AND t.trimestre = :tri
                    AND tt.slug = :slug
                    ORDER BY i.descripcion DESC';

        $query = $this->em->createQuery($dql);
        $query->setParameter('fechaDesde', $fechaDesde)
              ->setParameter('fechaHasta', $fechaHasta)
              ->setParameter('tri', $trimestre)
              ->setParameter('slug', $slug);
     ...
     * @return string
     */
    public function reportePedidoPorOrganismoyRubro($reparticion, $rubro, $pedido, $cantidad = 3)
    {
